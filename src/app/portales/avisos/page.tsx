...
const NOM: Record<string, string> = {
    ok: 'Listo para destacar', video: 'Falta video o tour', fotos: 'Faltan fotos',
    calidad: 'Calidad i24 baja', precio: 'Ajustar precio', comision: 'Comisión baja',
    revisar: 'Nadie busca esto aquí', ya_visible: 'Ya se ve sin pagar',
};
const ORDEN = ['ok', 'video', 'fotos', 'calidad', 'precio', 'comision', 'revisar', 'ya_visible'];
/** Se pueden destacar hoy o tras un arreglo que no cuesta presupuesto. */
const SE_PUEDEN = new Set(['ok', 'video', 'fotos', 'calidad', 'comision']);
...
export default function AvisosLive() {
    ...
    const porEstado = useMemo(() => {
        ...
        for (const a of d?.avisos ?? []) (m[a.estado] ??= []).push(a);
        return m;
    }, [d]);

    const filas = useMemo(() => {
        let f = d?.avisos ?? [];
        if (filtro) f = f.filter((a) => a.estado === filtro);
                ...
                <>
                    ...
                    <section className="mt-7 rounded-[2px] border border-neutral-200 bg-white p-5">
                        <div className="flex flex-wrap items-baseline gap-x-7 gap-y-3">
                            ...
                            <Dato n={String(porEstado.ok.length)} l="listos para destacar" />
                    ...
                    <Bloque titulo="Su inventario, aviso por aviso">
                        <div className="flex flex-wrap gap-2">
                            ...
                            {ORDEN.filter((k) => porEstado[k]?.length).map((k) => (
                                <Pill key={k} on={filtro === k} onClick={() => setFiltro(k)}>
                                    {NOM[k]} ({porEstado[k].length})
                                </Pill>
                            ))}
                        ...
                        <p className="mt-3 rounded-[2px] border-l-[3px] border-sea bg-light p-3 text-xs leading-relaxed">
                            Cada aviso dice <b>qué hay que hacer con él</b>. Poder destacarse no es
                            llevarse el lugar: eso depende del cupo de la cuenta y del orden por puntaje.
                        </p>
                        <div className="mt-3 max-h-[560px] overflow-auto rounded-[2px] border border-neutral-200">
                            <table className="w-full text-[12px]">
                                <thead className="sticky top-0 bg-white">
                                    <tr className="border-b border-neutral-200 text-left text-[9.5px] uppercase tracking-wider text-brand-gray">
                                        {['ID', 'Asesor', 'Colonia', 'Precio', '% com', 'Buscando',
                                          'Compet.', 'Leads/mes', 'Puntaje', 'Tier hoy', 'Qué hacer',
                                          'Qué le falta'].map((h) => (
                                ...
                                <tbody>
                                    {filas.map((a) => (
                                        <tr key={a.id} className="border-b border-neutral-100 hover:bg-light">
                                            ...
                                            <td className="px-2 py-1.5 text-right tabular-nums">{f1(a.leadsMes)}</td>
                                            ...
                                            <td className="px-2 py-1.5 whitespace-nowrap">
                                                <span className={`rounded-[3px] px-1.5 py-0.5 text-[10px] font-bold uppercase ${
                                                    SE_PUEDEN.has(a.estado)
                                                        ? 'bg-[#F6BE00]/25 text-[#8a6a00]'
                        ...
                        <p className="mt-3 text-[11px] text-brand-gray">
                            {filas.length} de {d.avisos.length} avisos · el puntaje ordena su inventario
                            (70 potencial×regalía + 15 comisión + 15 calidad de la cuenta). El percentil
                            se calcula <b>dentro de la cuenta</b>, así que no compara puntajes entre
                            inmobiliarias.
                        </p>
                    </Bloque>
                </>
